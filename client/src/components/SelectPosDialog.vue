...
const onDialogHide = () => {
  window.removeEventListener("keydown", handleKeydownOnposLocation);
  transaksiStore.posLocation = posLocation.value;
};

const transaksiStore = useTransaksiStore();
const posOptions = ref(["-"]);
const posLocation = ref("");
onMounted(async () => {
  posOptions.value = await transaksiStore.getLokasiPos();
  //   jenisKendaraanOptions.value = await transaksiStore.getJenisKendaraan();
});

const handleKeydownOnposLocation = (event) => {
  const key = event.key;
...
onMounted(async () => {
  //   console.log(posOptions.value.findIndex((index) => index));

  window.addEventListener("keydown", handleKeydownOnposLocation);
});
